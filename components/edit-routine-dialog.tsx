        ...
        <div className="px-8 pt-10 pb-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-10">
            ...
            <div className="flex flex-col gap-3">
              <label className="text-xl font-semibold text-[#24323d] tracking-tight">이름</label>
              <div className="rounded-xl bg-white border-none shadow-[inset_0_0_0_2px_rgba(11,92,122,0.12)] focus-within:shadow-[inset_0_0_0_2px_#0b5c7a] transition-all duration-300">
                <input
                  className="w-full bg-transparent border-none rounded-xl px-6 py-5 text-lg text-[#24323d] focus:ring-0 focus:outline-none placeholder-[#a8b4bf]"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-3">
                <label className="text-xl font-semibold text-[#24323d] tracking-tight">시작 시간</label>
                <TimePicker
                  value={time}
                  onChange={setTime}
                  open={timePickerOpen}
              ...
              <div className="flex flex-col gap-3">
                <label className="text-xl font-semibold text-[#24323d] tracking-tight">소요 시간 (분)</label>
                <div className="rounded-xl bg-white shadow-[inset_0_0_0_2px_rgba(11,92,122,0.12)] focus-within:shadow-[inset_0_0_0_2px_#0b5c7a] transition-all">
                  <input
                    type="number"
                    min={1}
                    max={180}
                    className="w-full bg-transparent border-none rounded-xl px-6 py-5 text-lg text-[#24323d] focus:ring-0 focus:outline-none"
                    value={duration}
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-end">
                <label className="text-xl font-semibold text-[#24323d] tracking-tight">스케줄</label>
                <span className="text-sm text-[#6a7a8a] bg-[#e8eff5] px-3 py-1 rounded-full">
                  {selectedDays.filter(Boolean).length === 7 ? "매일" : `주 ${selectedDays.filter(Boolean).length}회`}
                </span>
              </div>
              <div className="flex justify-between items-center bg-[#e8eff5] p-4 rounded-xl">
                {dayLabels.map((day, i) => (
                  <button
                    key={day}
                    type="button"
                    ...
                    className={`w-11 h-11 rounded-full font-semibold text-base flex items-center justify-center transition-all duration-300 ${
                      selectedDays[i]
                        ? "bg-[#0b5c7a] text-[#ffffff] shadow-sm hover:opacity-90"
                        : "bg-[#cfdce6] text-[#6a7a8a] hover:bg-[#e8eff5]"
                    }`}
                  >
                    {day}
                  </button>
            ...
            {/* 스마트 리마인더 */}
            <div className="flex flex-col gap-4 bg-[#e8eff5] p-6 rounded-xl relative overflow-hidden">
              <div className="absolute -right-10 -top-10 w-32 h-32 bg-[#b8dcec] rounded-full opacity-20 blur-2xl pointer-events-none" />
              <div className="flex justify-between items-center relative z-10">
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-[#0b5c7a] text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>notifications_active</span>
                  <h3 className="text-xl font-semibold text-[#24323d] tracking-tight">스마트 리마인더</h3>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div className="w-14 h-8 bg-[#cfdce6] rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:border-[#a8b4bf]/20 after:border after:rounded-full after:h-6 after:w-6 after:transition-all after:shadow-sm peer-checked:bg-[#0b5c7a]" />
                </label>
              </div>
              <p className="text-base text-[#6a7a8a] leading-relaxed mt-1 max-w-[90%] relative z-10">
